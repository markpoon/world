...
Crafty.extend isometric2:
  ...
  init: (width, height) ->
    @_tile.width = width
    @_tile.height = (if height > 0 then height else width / 2) #Setup width/2 if height isn't set
    this
  ...
  ###
  pos2px: (x, y) ->
    left:((x - y) * @_tile.height)
    top: ((x + y) * @_tile.height/2)
  ...
  ###
  px2pos: (left, top) ->
    x: ((top + left) / @_tile.height)
    y: ((top - left) / @_tile.height*2)
  ...
  centerAt: (x, y) ->
    if typeof x is "number" and typeof y is "number"
      center = @pos2px(x, y)
      Crafty.viewport._x = -center.left + Crafty.viewport.width / 2 - @_tile.width / 2
      Crafty.viewport._y = -center.top + Crafty.viewport.height / 2 - @_tile.height / 2
      this
    else
      top: -Crafty.viewport._y + Crafty.viewport.height / 2 - @_tile.height / 2
      left: -Crafty.viewport._x + Crafty.viewport.width / 2 - @_tile.width / 2
  ...
  area: ->
    ...
    center = @centerAt()
    start = @px2pos(-center.left + Crafty.viewport.width / 2, -center.top + Crafty.viewport.height / 2)
    end = @px2pos(-center.left - Crafty.viewport.width / 2, -center.top - Crafty.viewport.height / 2)
    x:
      start: start.x
      end: end.x
